...
class RecentPaymentAmountAdapter(private val list: ArrayList<Double>,
                           private val context: Context,
                           private val recentPayment: RecentPaymentAmountManager): RecyclerView.Adapter<RecentPaymentAmountAdapter.ViewHolder>() {

    inner class ViewHolder(item: View): RecyclerView.ViewHolder(item) {
        private val binding = WalletItemRecentPaymentBinding.bind(item)
        fun bind(amount: Double) {
            binding.title.text = amount.toString().replace(".", ",")

            binding.itemView.setOnClickListener {
                recentPayment.onClick(amount)
            }
        }
    }
